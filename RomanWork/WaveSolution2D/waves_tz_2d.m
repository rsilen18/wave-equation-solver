...
function [u,e] = waves_tz_2d(def,cfl,plot_flag,order)
    ...
    n = 2;
    while n*dt <= def.t_f
        unp1 = main_time_step(def,sigma_x,sigma_y,x,y,order,dt,unm1,un,n-1);
...
% time steps for n >= 2
function unp1 = main_time_step(def,sigma_x,sigma_y,x,y,order,dt,unm1,un,n)
    nx = size(x,2);
    ny = size(y,2);
    unp1 = zeros(nx,ny);
    for i = 1+order/2:nx-order/2
...
%             unp1(i,j) = v(x(i),y(j),n*dt);
            if (order >= 4)
                unp1(i,j) = unp1(i,j) + ...
                            ...
                            def.c^2*dt^4/12*(h_xx(def,x(i),y(j),n*dt)+h_yy(def,x(i),y(j),n*dt)) + ...
                            2*dt^4/12*h_tt(def,x(i),y(j),n*dt);
            end
        end
    end
end